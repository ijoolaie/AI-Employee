...
| AgentTemplate | `backend/app/models/agent_template.py`, `p8_04_agent_governance_foundation.py` | `backend/app/api/v1/agent_templates.py` + lifecycle service + evaluation evidence API | GOVERNANCE API IMPLEMENTED / E2E GATES PENDING |
...
| Agent identity / sponsorship | `AgentIdentity`, `AgentAccessReview`, `AgentInstance.sponsor_user_id` | governed provisioning + identity/access-review APIs | ENFORCEMENT IMPLEMENTED / UI + E2E PENDING |
| Agent execution authorization | `agent_governance.py`, `agent_tool_governance.py`, canonical Run `agent_instance_id`, worker checks | ToolRegistry boundary + Run worker enforcement | IMPLEMENTED / E2E GATES PENDING |
| Workforce Registry | `agent_workforce_registry.py` projection over Definition → Template → Instance + Run cost attribution | `GET /agent-governance/workforce-registry` | API IMPLEMENTED / GOVERNANCE UI PENDING |
...
| Scoped Agent tools | V1.5 specification; `agent_tool_governance.py` + ToolRegistry boundary | developer/API surfaces exist | ENFORCEMENT IMPLEMENTED / E2E PENDING |
...
| Usage / cost attribution | existing usage/billing concepts + Run `total_cost_usd` projection by AgentInstance | workforce registry exposes aggregate Agent cost | PARTIAL / VERIFY GATES |
...
These must not be merged into one claim. The Stage 8 governance foundation, persisted evaluation/access-review evidence, canonical Agent Run binding, centralized ToolRegistry enforcement boundary, and Workforce Registry API are real repository implementation; UI and end-to-end evidence remain separate gates.
...
- Stage 8 implementation is active with governed `AgentDefinition → AgentTemplate → AgentInstance` persistence and lifecycle/provisioning/execution-governance API surfaces.
...
1. Harden the centralized ToolRegistry enforcement with full integration/e2e evidence for revocation, expiry, retirement, tenant isolation and permission/tool allow-list denial.
2. Add Governance UI for Workforce Registry, identity/access review, evaluation evidence and lifecycle controls.
3. Add customer installation/e2e tests covering tenant isolation, sponsorship, approval attribution and lifecycle transitions.
4. Add dynamic workforce proposal → Board review → CEO approval workflow.
