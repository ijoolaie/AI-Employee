...
The v1.4.9 release has passed repository engineering gates and exact-SHA Production Certification. Post-certification mainline now includes Employee Template Catalog, Product/Customer/Order/Invoice lifecycle parity, edition-aware Test Center execution, non-destructive schedule lifecycle UI, and customer fa/en RTL browser acceptance. PRs #607–#613 relevant to this slice passed the required CI/security/production-like gates before merge. Do not restart completed test suites without a regression trigger.
