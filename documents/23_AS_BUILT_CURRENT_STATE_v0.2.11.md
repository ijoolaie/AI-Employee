# Current As-Built Addendum — v0.2.11-LMSTUDIO
...
- Successful LM Studio/Gemma end-to-end Run after the asyncpg worker-session fix. The observed response was `Understood. Test acknowledged.`; provider usage was 51 prompt + 7 completion tokens; cost was `0.0 USD`; the Run and audit records committed successfully.

## AI Gateway observability correction

The v0.2.10 Gateway read `Timer.duration_ms` before the Timer context exited, producing `latency_ms=0` in the successful E2E log. v0.2.11 adds a live `Timer.elapsed_ms` measurement and enriches `ChatResult` with Gateway-owned `latency_ms` and `cost_usd`. Run execution consumes `result.cost_usd` instead of recalculating provider cost.
...
- Successful LM Studio/Gemma Run after fix: **PASS**
