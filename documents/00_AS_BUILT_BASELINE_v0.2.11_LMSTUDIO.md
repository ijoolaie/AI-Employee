...
### Verified
- Successful end-to-end Gemma call through LM Studio completed after the Windows asyncpg worker-session fix.
- `POST /api/v1/runs` created the Run with HTTP 201; Celery executed it successfully; LM Studio returned HTTP 200; the Run reached `success`; output text was persisted; 51 prompt + 7 completion = 58 tokens were recorded; provider cost was `0.0 USD`; audit logs were persisted; and the transaction committed successfully.
- AI Provider Call latency tracking was corrected in v0.2.11 so the measured provider-call duration is captured before the Timer context exits.
...
## 8. Observability correction in v0.2.11

The v0.2.10 implementation recorded `latency_ms=0` because the Gateway read `Timer.duration_ms` inside a `finally` block before the context manager's `__exit__` finalized it. v0.2.11 adds `Timer.elapsed_ms` for live measurements, stores the resulting latency on `AIProviderCall`, and carries Gateway-calculated `cost_usd`/`latency_ms` on `ChatResult`. `Run.total_cost_usd` now consumes the Gateway value rather than recalculating provider cost.
...
## 11. Release delta from v0.2.10-LMSTUDIO
