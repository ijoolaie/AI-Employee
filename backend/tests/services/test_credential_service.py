...
async def test_store_credential_encrypts_secret_and_returns_opaque_ref(monkeypatch):
    ...
    credential = await credential_service.store_credential(
        Db(), tenant_id=tenant_id, provider="shopify", name="shopify:access_token", secret="super-secret"
    )
...
async def test_resolve_credential_rejects_expired_credential():
    credential = Credential(
        id=uuid4(), tenant_id=uuid4(), provider="x", name="x", ciphertext="bad", status="active", active=True,
...
async def test_agent_credential_access_requires_policy(monkeypatch):
    ...
    async def _deny(*args, **kwargs):
        raise ValidationAppError("permission denied")
    ...
    with pytest.raises(ValidationAppError, match="permission denied"):
        await credential_service.resolve_credential(
            Db(), tenant_id=tenant_id, credential_id=credential_id, agent_instance_id=agent_id, run_id=run_id
        )
