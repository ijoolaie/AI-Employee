...
async def test_suspended_agent_cannot_execute():
    ...
    instance = SimpleNamespace(
        id=uuid4(), tenant_id=tenant_id, status=AgentInstanceStatus.SUSPENDED,
        enabled=False, permission_policy={"allowed_tools": ["ticket.read"], "permissions": ["tickets.read"]},
    )
    ...
    with pytest.raises(ValidationAppError, match="agent_instance_not_executable"):
        await assert_agent_can_execute(_GovernanceDB(instance, identity), tenant_id=tenant_id, agent_instance_id=instance.id,
                                       tool_name="ticket.read", required_permission="tickets.read")
...
async def test_revoked_agent_identity_cannot_execute():
    tenant_id = uuid4()
    instance = SimpleNamespace(
        id=uuid4(), tenant_id=tenant_id, status=AgentInstanceStatus.ENABLED,
        enabled=True, permission_policy={"allowed_tools": ["ticket.read"], "permissions": ["tickets.read"]},
    )
...
async def test_expired_agent_identity_is_deactivated_and_cannot_execute():
    ...
    identity = SimpleNamespace(active=True, revoked_at=None, expires_at=datetime.now(timezone.utc) - timedelta(minutes=1))
    db = _GovernanceDB(instance, identity)
    with pytest.raises(ValidationAppError, match="expired"):
        await assert_agent_can_execute(db, tenant_id=tenant_id, agent_instance_id=instance.id,
                                       tool_name="ticket.read", required_permission="tickets.read")
...
async def test_successful_agent_work_item_creates_agent_attributed_run(monkeypatch):
    tenant_id = uuid4(); agent_id = uuid4(); run_id = uuid4(); definition_id = uuid4(); employee_id = uuid4(); version_id = uuid4(); requester_id = uuid4(); calls = {}
