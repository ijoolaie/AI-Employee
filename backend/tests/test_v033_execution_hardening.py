from app.schemas.workflow import WorkflowCreate, WorkflowRunCreate, WorkflowStepDefinition, WorkflowParallelBranch
...
def test_parallel_workflow_schema():
    payload = WorkflowCreate(
        slug="parallel-flow", name="Parallel Flow",
        steps=[WorkflowStepDefinition(
            key="parallel", type="parallel", branches=[
                WorkflowParallelBranch(key="a", steps=[{"key":"a1", "type":"employee", "employee_id":"8740db8a-87b5-4f68-aa52-b270d96e0555"}]),
                WorkflowParallelBranch(key="b", steps=[{"key":"b1", "type":"employee", "employee_id":"8740db8a-87b5-4f68-aa52-b270d96e0555"}]),
            ]
        )]
    )
