...
def test_internal_manager_ceo_report_has_exact_semantic_tool_binding() -> None:
    from app.services.ai_workforce_roles import get_workforce_capability_contract

    contract = get_workforce_capability_contract("ai_internal_manager", "prepare_ceo_report")
    assert contract.tool_names == ("workforce_prepare_ceo_report",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False


def test_trader_risk_analysis_has_exact_semantic_tool_binding() -> None:
    from app.services.ai_workforce_roles import get_workforce_capability_contract

    contract = get_workforce_capability_contract("ai_trader", "risk_analysis")
    assert contract.tool_names == ("workforce_market_risk_analysis",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False
...
def test_marketing_coordinate_content_has_exact_semantic_tool_binding() -> None:
    from app.services.ai_workforce_roles import get_workforce_capability_contract

    contract = get_workforce_capability_contract(
        "ai_marketing_advertising_manager", "coordinate_content"
    )
    assert contract.tool_names == ("workforce_coordinate_content",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False
...
def test_internal_manager_cost_optimization_has_exact_semantic_tool_binding():
    ...
    contract = get_workforce_capability_contract(
        "ai_internal_manager", "prepare_cost_optimization"
    )
    assert contract.tool_names == ("workforce_prepare_cost_optimization",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False
...
def test_internal_manager_reprioritize_task_has_exact_semantic_tool_binding():
    ...
    contract = get_workforce_capability_contract(
        "ai_internal_manager", "reprioritize_task"
    )
    assert contract.tool_names == ("workforce_reprioritize_task",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False


def test_internal_manager_assign_task_has_exact_semantic_tool_binding():
    from app.services.ai_workforce_roles import get_workforce_capability_contract

    contract = get_workforce_capability_contract(
        "ai_internal_manager", "assign_task"
    )
    assert contract.tool_names == ("workforce_assign_task",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False
...
def test_internal_manager_capacity_request_has_exact_semantic_tool_binding():
    ...
    contract = get_workforce_capability_contract(
        "ai_internal_manager", "request_workforce_capacity"
    )
    assert contract.tool_names == ("workforce_request_capacity",)
    assert contract.required_permissions == ("run.execute",)
    assert contract.approval_required is False
