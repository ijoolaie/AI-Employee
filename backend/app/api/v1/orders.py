...
async def list_orders(
    ctx: OrdersReadContext,
    db: DbSession,
    status: str | None = Query(default=None),
):
    rows = await order_service.list_orders(db, tenant_id=ctx.tenant_id, status=status)
    return APIResponse(success=True, data=[BusinessOrderResponse.model_validate(r) for r in rows])


@router.get("/summary", response_model=APIResponse[OrderSummary])
async def summary(ctx: OrdersReadContext, db: DbSession):
...
async def get_order(order_id: UUID, ctx: OrdersReadContext, db: DbSession):
    order = await order_service.get_order(db, tenant_id=ctx.tenant_id, order_id=str(order_id))
