...
async def _get_work_item(db: AsyncSession, work_item_id: UUID, tenant_id: UUID, *, for_update: bool = False) -> WorkItem:
    if for_update:
        stmt = select(WorkItem).where(WorkItem.id == work_item_id, WorkItem.tenant_id == tenant_id).with_for_update()
...
def _summary(item: WorkItem) -> WorkItemSummary:
    return WorkItemSummary(id=item.id, title=item.title, description=item.description, status=item.status.value, priority=item.priority, requester_id=item.requester_id, executor_type=item.executor_type.value if item.executor_type else None, executor_id=item.executor_id, input_data=item.input_data or {}, output_data=item.output_data, created_at=item.created_at, updated_at=item.updated_at)
...
async def list_work_items(status_filter: str | None = Query(default=None, alias="status"), limit: int = Query(default=100, ge=1, le=200), db: AsyncSession = Depends(get_db), current_user=Depends(get_current_context)):
...
async def history(work_item_id: UUID, limit: int = Query(default=100, ge=1, le=200), db: AsyncSession = Depends(get_db), current_user=Depends(get_current_context)):
    ...
    stmt = select(AuditLog).where(AuditLog.tenant_id == current_user.tenant_id, AuditLog.resource_type == "work_item", AuditLog.resource_id == str(work_item_id)).order_by(AuditLog.created_at.desc()).limit(limit)
    result = await db.execute(stmt)
...
async def assign_agent(work_item_id: UUID, payload: AgentAssignmentRequest, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_context)):
    try:
        item = await assign_agent_work_item(db, tenant_id=current_user.tenant_id, work_item_id=work_item_id, agent_instance_id=payload.agent_instance_id)
        await record_execution_event(db, tenant_id=item.tenant_id, work_item_id=item.id, action="work_item.assigned", actor_type="user", actor_id=current_user.user_id, metadata={"executor_type": "agent", "agent_instance_id": str(payload.agent_instance_id)})
        await db.commit()
    except ExecutionError as exc:
        await db.rollback()
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
