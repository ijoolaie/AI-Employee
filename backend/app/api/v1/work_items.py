...
@router.get(
    "",
    response_model=list[WorkItemSummary],
...
async def list_work_items(
    ...
    stmt = (
        select(WorkItem)
        .where(WorkItem.tenant_id == current_user.tenant_id)
    ...
    if status_filter:
        try:
            ...
            raise HTTPException(
                ...
                detail="invalid work item status",
            ) from exc
...
async def assign_human(
    ...
    if executor is None or executor.tenant_id != current_user.tenant_id or not executor.is_active:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="human executor not found",
        )
    try:
...
async def assign_agent(
    ...
    try:
        item = await assign_agent_work_item(
            db,
        ...
        await db.rollback()
        if str(exc) in {"agent instance not found", "work item not found"}:
...
async def dispatch(
    ...
    try:
        result = await UnifiedExecutionService(
            db,
        ...
        await record_execution_event(
            ...
            metadata={"status": item.status.value, **(item.output_data or {})},
        )
        await db.commit()
    except ExecutionError as exc:
        await record_execution_event(
            ...
            tenant_id=item.tenant_id,
            work_item_id=item.id,
            action="work_item.execution_failed",
            actor_type="user",
            actor_id=current_user.user_id,
            status="failure",
...
async def retry(
    ...
    try:
        ...
        await record_execution_event(
            ...
            metadata={
                "status": item.status.value,
                "retry_count": (item.policy_context or {}).get("retry_count", 0),
            },
        )
        await db.commit()
    except ExecutionError as exc:
