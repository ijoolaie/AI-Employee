...
class WorkflowParallelBranchRun(Base):
    ...
    completed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    __table_args__ = (UniqueConstraint("workflow_step_run_id", "branch_key", name="uq_workflow_parallel_branch"), Index("ix_parallel_branch_workflow_run", "workflow_run_id"), Index("ix_parallel_branch_step_run", "workflow_step_run_id"), Index("ix_parallel_branch_lease_expires_at", "execution_lease_expires_at"))
