    ...
    async def dispatch(self, work_item: WorkItem) -> ExecutionResult:
        ...
        try:
            if work_item.executor_type is ExecutorType.HUMAN:
                ...
                # Dispatch only makes the human work active. Completion is an
                # explicit operation so the platform never reports work as
                # completed before the human actually finishes it.
                work_item.status = WorkItemStatus.RUNNING
