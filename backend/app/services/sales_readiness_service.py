...
TEMPLATES = [
    {
        ...
        "input_contract_fa": "نیاز مشتری، جست‌وجوی محصول یا درخواست خرید.",
        "output_contract": "Product recommendations, availability, and an approved order request.",
        "output_contract_fa": "پیشنهاد محصول، وضعیت موجودی و درخواست سفارش نیازمند تأیید.",
        "dependencies": "Product catalog", "Inventory",
        "dependencies_fa": ["کاتالوگ محصولات", "موجودی"],
    ...
    },
    {
        ...
        "category": "support",
        "category_fa": "پشتیبانی",
        "input_contract": "Customer question plus an order ID or order number when relevant.",
        "input_contract_fa": "پرسش مشتری به همراه شناسه یا شماره سفارش در صورت نیاز.",
        "output_contract": "Verified order status, delivery information, or a human handoff.",
        "output_contract_fa": "وضعیت تأییدشده سفارش، اطلاعات ارسال یا ارجاع به نیروی انسانی.",
        "dependencies": "Order data",
        "dependencies_fa": ["اطلاعات سفارش"],
    ...
    },
    {
        ...
        "category": "orders",
        "category_fa": "سفارش‌ها",
        "input_contract": "Order ID or customer-visible order number.",
        "input_contract_fa": "شناسه سفارش یا شماره قابل مشاهده برای مشتری.",
        "output_contract": "Current order status and requested delivery information.",
        "output_contract_fa": "وضعیت فعلی سفارش و اطلاعات تحویل درخواستی.",
        "dependencies": "Order data",
        "dependencies_fa": ["اطلاعات سفارش"],
    ...
    },
    {
        ...
        "input_contract_fa": "نام محصول، SKU، دسته‌بندی یا پرسش درباره موجودی.",
        "output_contract": "Verified product attributes, price, and inventory status.",
        "output_contract_fa": "ویژگی‌های تأییدشده محصول، قیمت و وضعیت موجودی.",
        "dependencies": "Product catalog", "Inventory",
        "dependencies_fa": ["کاتالوگ محصولات", "موجودی"],
    ...
    },
    {
        ...
        "category": "analytics",
        "category_fa": "تحلیل و گزارش",
        "input_contract": "Tenant file ID for a CSV or Excel dataset.",
        "input_contract_fa": "شناسه فایل تننت برای داده CSV یا Excel.",
        "output_contract": "Validated dataset analysis plus generated report artifacts.",
        "output_contract_fa": "تحلیل اعتبارسنجی‌شده داده به همراه فایل‌های گزارش تولیدشده.",
        "dependencies": "Uploaded CSV/Excel file",
        "dependencies_fa": ["فایل CSV/Excel بارگذاری‌شده"],
    ...
    },
    {
        ...
        "input_contract_fa": "شناسه فایل تننت برای یک سند پشتیبانی‌شده.",
        "output_contract": "Extracted text, document classification, and detected field candidates.",
        "output_contract_fa": "متن استخراج‌شده، دسته‌بندی سند و فیلدهای شناسایی‌شده.",
        "dependencies": "Uploaded document",
        "dependencies_fa": ["سند بارگذاری‌شده"],
    ...
    },
    {
        ...
        "input_contract_fa": "اطلاعات مشتری و اقلام، شناسه فاکتور یا درخواست خلاصه مالی.",
        "output_contract": "Invoice result, status update, PDF artifact, or tenant financial summary.",
        "output_contract_fa": "نتیجه فاکتور، به‌روزرسانی وضعیت، فایل PDF یا خلاصه مالی تننت.",
        "dependencies": "Customer/order context", "Invoice data",
        "dependencies_fa": ["زمینه مشتری/سفارش", "اطلاعات فاکتور"],
