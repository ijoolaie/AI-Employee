...
async def provision_edition_admin(db: AsyncSession, tenant: Tenant, user: User) -> Role:
    permissions = RESELLER_PERMISSIONS if tenant.tenant_kind == EDITION_RESELLER else CUSTOMER_PERMISSIONS
    return await _ensure_role(db, tenant, "Admin", permissions)
