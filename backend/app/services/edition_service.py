...
RESELLER_PERMISSIONS = {
    ...
    "reseller.entitlement.delegate", "support.escalation.create", "team.install", "team.execute", "team.evaluate",
...
CUSTOMER_PERMISSIONS = {
    ...
    "workflow.approval.read", "workflow.approval.decide", "workflow.event.read", "workflow.event.write",
    "memory.read", "memory.write", "memory.delete", "feedback.create", "feedback.read",
    "support.escalation.create", "team.install", "team.execute", "team.evaluate",
    "marketplace.publish", "marketplace.read",
}
...
async def provision_child_tenant(db: AsyncSession, *, parent: Tenant, name: str, slug: str, admin_email: str, admin_password: str, full_name: str | None, kind: str, vendor_release_tag: str | None, delivery_revision: str | None) -> tuple[Tenant, User]:
    ...
    await record_audit(db, tenant_id=parent.id, actor_id=None, action=f"edition.{kind}.provisioned", resource_type="tenant", resource_id=str(tenant.id), metadata={"child_tenant_id": str(tenant.id), "child_kind": kind, "delivery_revision": delivery_revision})
    await db.commit()
...
async def _authorized_parent_entitlement(db: AsyncSession, *, parent: Tenant, feature_code: str, requested_quota: int | None) -> int | None:
    if parent.tenant_kind == EDITION_VENDOR:
        return requested_quota
    row = (await db.execute(select(TenantEntitlement).where(TenantEntitlement.tenant_id == parent.id, TenantEntitlement.feature_code == feature_code, TenantEntitlement.is_enabled.is_(True)))).scalar_one_or_none()
    if row is None:
...
async def delegate_entitlement(db: AsyncSession, *, parent: Tenant, child: Tenant, feature_code: str, quota_limit: int | None) -> TenantEntitlement:
...
async def create_support_escalation(db: AsyncSession, *, from_tenant: Tenant, opened_by: UUID, subject: str, description: str) -> SupportEscalation:
...
async def record_audit(db: AsyncSession, *, tenant_id, actor_id, action: str, resource_type: str, resource_id: str, metadata: dict) -> None:
    db.add(AuditLog(tenant_id=tenant_id, actor_type="user" if actor_id else "system", actor_id=actor_id, action=action, resource_type=resource_type, resource_id=resource_id, request_id=request_id_var.get(), status="success", metadata=metadata))
    await db.commit()
