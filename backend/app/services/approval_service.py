"""Human approval lifecycle for gated Tool calls."""
...
async def create_request(
    ...
    *,
    ...
    await audit_service.record(
        ...
        metadata={"approval_id": str(approval.id), "tool": tool_name, "tool_call_id": tool_call_id},
    )
    return approval
...
async def decide(
    ...
    *,
    ...
    result = await db.execute(
        ...
        .where(ToolApprovalRequest.id == approval_id, ToolApprovalRequest.tenant_id == tenant_id)
        .with_for_update()
    )
    ...
    run_result = await db.execute(
        select(Run).where(Run.id == approval.run_id, Run.tenant_id == tenant_id).with_for_update()
    )
    ...
    if approval.status == "rejected":
        run.status = "failed"
        run.error = {"code": "TOOL_APPROVAL_REJECTED", "message": reason or "Human approval rejected the tool call."}
    else:
        run.status = "pending"
