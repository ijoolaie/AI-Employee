...
async def _cfg(db: AsyncSession, integration: CommerceIntegration):
    ...
    ref = (cfg.get("credential_refs") or {}).get("access_token")
    if not shop or not ref:
        raise ValidationAppError("Shopify requires an installed shop and credential reference")
    if not str(ref).startswith("cred:"):
        raise ValidationAppError("Shopify access token must use a credential reference")
    try:
        credential_id = uuid.UUID(str(ref)[5:])
    except (ValueError, AttributeError) as exc:
        raise ValidationAppError("Invalid Shopify credential reference") from exc
    token = await resolve_credential(db, tenant_id=integration.tenant_id, credential_id=credential_id, tool_name="shopify.graphql")
    version = str(cfg.get("api_version") or get_settings().shopify_api_version).strip()
    return shop, token, version

async def _graphql(db: AsyncSession, integration: CommerceIntegration, query: str, variables: dict[str, Any] | None = None):
    shop, token, version = await _cfg(db, integration)
...
async def _paginate(db, integration, query, root, variables=None, page_size=100, max_pages=50):
    variables = dict(variables or {}); variables["first"] = page_size; variables["after"] = None; items = []
    for _ in range(max_pages):
        data = await _graphql(db, integration, query, variables); conn = data[root]; items.extend(conn.get("nodes") or [])
...
async def sync_orders(db, tenant_id, integration_id):
    ...
    for remote in orders:
        ...
        if not customer: customer = Customer(tenant_id=tenant_id, external_key=key, name=" ".join(filter(None, [c.get("firstName"), c.get("lastName")])) or None, email=c.get("email"), phone=c.get("phone"), last_channel="shopify"); db.add(customer); await db.flush()
        lines = [{"description": item.get("title") or "Item", "quantity": item.get("quantity") or 1, "unit_price": float((((item.get("originalUnitPriceSet") or {}).get("shopMoney") or {}).get("amount") or 0)), "sku": item.get("sku")} for item in ((remote.get("lineItems") or {}).get("nodes") or [])]
