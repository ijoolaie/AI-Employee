...
async def _assign_tenant_admin_role(db: AsyncSession, user: User, tenant_id: UUID) -> Role:
    ...
    for code in DEFAULT_TENANT_ADMIN_PERMISSIONS:
        ...
        if permission is None:
            ...
            db.add(permission)
            await db.flush()
        # Do not access role.permissions here. With AsyncSession, a newly
        # created relationship collection may trigger a lazy/implicit IO
        # path and raise MissingGreenlet. The association tables are the
        # authoritative RBAC links, so write them explicitly.
        await db.execute(
    ...
    # Same principle for the user-role link: avoid touching user.roles in the
    # registration transaction because it may require an implicit async load.
    await db.execute(
        insert(user_roles)
...
async def register_tenant_and_user(
    ...
    # Email unique within tenant (and we check globally for simplicity in v1)
    normalized_email = payload.email.lower()
    existing_user = await db.execute(
        select(User).where(User.email == normalized_email, User.tenant_id == tenant.id)
    )
    ...
    user = User(
        ...
        is_active=True,
        is_superuser=True,  # first user of tenant is admin
    )
    db.add(user)
    await db.flush()
    ...
    await audit_service.record(
        ...
        metadata={"role": "Admin", "user_id": str(user.id)},
    )
    ...
    await audit_service.record(
        ...
        request_id=request_id_var.get(),
        metadata={"tenant_slug": tenant.slug, "user_email": user.email},
    )
    return tenant, user
...
async def authenticate_user(db: AsyncSession, payload: LoginRequest) -> User:
    ...
    if user is None or not user.is_active:
        await audit_service.record(
            ...
            status="failure",
            request_id=request_id_var.get(),
            metadata={"email": payload.email.lower(), "reason": "user_not_found_or_inactive"},
        )
        raise UnauthorizedError("Invalid credentials")
