...
async def _get_or_create_stripe_customer(
    ...
    *,
    ...
    # Recover an already-created Customer after the provider idempotency window
    # has expired and the local DB write was lost. Search is intentionally a
    # reconciliation primitive only: Stripe documents that Customer Search is
    # eventually consistent, so a miss must not be treated as proof of absence.
    matches = stripe.Customer.search(
    ...
    customer = stripe.Customer.create(
        ...
        # Stable tenant-scoped idempotency remains the final protection when
        # reconciliation has no indexed match. It covers concurrent/retried
        # creates while Stripe retains the idempotency key.
...
async def create_checkout_session(db: AsyncSession, *, tenant_id: uuid.UUID, user_id: uuid.UUID, plan_code: str, idempotency_key: str) -> str:
    ...
    session = stripe.checkout.Session.create(
        ...
        success_url=settings.stripe_checkout_success_url, cancel_url=settings.stripe_checkout_cancel_url,
        client_reference_id=str(tenant_id), metadata={"tenant_id": str(tenant_id), "plan_code": plan_code},
...
async def create_portal_session(db: AsyncSession, *, tenant_id: uuid.UUID) -> str:
    ...
    if not sub.provider_customer_id:
        raise ConflictError("No Stripe customer on file for this tenant yet — complete a Checkout session first.")
    portal = stripe.billing_portal.Session.create(customer=sub.provider_customer_id, return_url=settings.stripe_portal_return_url)
    return portal.url
...
async def apply_webhook_event(db: AsyncSession, event) -> dict:
    ...
    if existing is not None:
        return {"event_id": str(existing.id), "stripe_event_id": provider_event_id, "event_type": existing.event_type, "duplicate": True}
    ...
    if event_type == "checkout.session.completed":
        ...
        if tenant_ref:
            ...
            existing = (await db.execute(select(Subscription).where(Subscription.provider_subscription_id == stripe_sub_id))).scalar_one_or_none()
            tenant_id = existing.tenant_id if existing else None
        stripe_status = data.get("status")
        status = {"active": "active", "trialing": "trialing", "past_due": "past_due", "canceled": "canceled", "unpaid": "past_due"}.get(stripe_status, None)
