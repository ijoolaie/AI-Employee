  ...
  return <>
    ...
    <div className="grid gap-6 p-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-6">
        ...
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Wrench className="h-4 w-4 text-brand-600"/>{m.allowedTools}</CardTitle></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2">{tools.isLoading ? <Spinner/> : (tools.data ?? []).map(t => <button key={t.name} onClick={()=>toggle(t.name)} className={`rounded-xl border p-4 text-start transition ${selectedTools.includes(t.name)?"border-brand-400 bg-brand-50":"border-gray-200 hover:bg-gray-50"}`}><div className="flex items-center justify-between gap-2"><span className="font-medium text-sm">{t.name}</span>{selectedTools.includes(t.name)&&<Check className="h-4 w-4 text-brand-600"/>}</div><p className="mt-1 text-xs text-gray-500">{t.description}</p><p className="mt-2 text-[11px] text-gray-400">{t.side_effects?m.sideEffect:m.safeTool}</p></button>)}{!(tools.data ?? []).length && <p className="text-sm text-gray-500">{m.noTools}</p>}</div></CardContent></Card>
...
function Toggle({label,description,value,onChange,disabled=false}:{label:string;description:string;value:boolean;onChange:(v:boolean)=>void;disabled?:boolean}){return <button type="button" disabled={disabled} onClick={()=>onChange(!value)} className="flex w-full items-start justify-between gap-4 rounded-xl border border-gray-200 p-4 text-start disabled:opacity-60"><span><span className="block text-sm font-medium text-gray-900">{label}</span><span className="mt-1 block text-xs leading-5 text-gray-500">{description}</span></span><span className={`mt-0.5 flex h-6 w-11 shrink-0 rounded-full p-1 transition ${value?"bg-brand-600":"bg-gray-200"}`}><span className={`h-4 w-4 rounded-full bg-white shadow transition ${value?"translate-x-5":"translate-x-0"}`}/></span></button>}
