    ...
    <div className="space-y-4 p-6">
      ...
      {item && <>
        ...
        <section className="space-y-4 rounded-xl border bg-white p-5 shadow-sm">
          ...
          <p className="text-sm text-gray-700">{item.description ?? String(item.input_data?.message ?? "Work item")}</p>
          <dl className="grid gap-3 text-sm sm:grid-cols-2">
            ...
            <div><dt className="text-gray-500">{m.status}</dt><dd>{item.status}</dd></div>
          </dl>
        </section>
        ...
        {runId && <section className="space-y-4 rounded-xl border bg-white p-5 shadow-sm">
          ...
          {approval && <div className="space-y-3">
            <div className="text-sm"><span className="font-medium">{approval.tool_name}</span><span className="ms-2 text-gray-500">{approval.status}</span></div>
        ...
        <section className="space-y-4 rounded-xl border bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-3 text-sm sm:grid-cols-2">
            <div><dt className="text-gray-500">{m.status}</dt><dd className="font-medium">{item.status}</dd></div>
            <div><dt className="text-gray-500">{m.executor}</dt><dd>{item.executor_type ?? m.unassigned}</dd></div>
          </div>
          ...
          <div className="space-y-3">
            {historyQuery.data?.map((event: WorkItemHistoryEvent) => (
              <div key={event.id} className="rounded-lg border p-3">
                ...
                <p className="mt-1 text-xs text-gray-500">{event.actor_type} · {event.status}{event.request_id ? ` · ${event.request_id}` : ""}</p>
              </div>
            ))}
          </div>
        </section>
